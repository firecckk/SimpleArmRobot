...
Vector3 state::debug = {-1, 0, 0};

float state::jointAngles[] = {0, 0, 0, 0, 0, 0};
bool state::grab = false;
bool state::require_update = false; // will be rename to arm update
bool state::move_update = false;
...
float state::get_move_roll() {
    return state::movement.z;
}
